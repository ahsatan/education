...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname P5_championship_bracket) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
; 
...
; .
; 
; (Source: http://scores.usaultimate.org/scores/#womens/tournament/13774)
; 


(define Bracket (signature (mixed Result Boolean)))
;; (ResultOf String String Bracket Bracket)
(define-struct result (team-won team-lost br-won br-lost))
;; interp. A tournament competition bracket.
;;    #false indicates an empty bracket.
;;    - team-won beat team-lost
;;    - br-won represents team-won's bracket prior to this match
;;    - br-lost represents team-lost's bracket prior to this match
  
(define B0 false)  ; empty tournament bracket
...
; 
; Use the local expression form to improve the function.
; 


(: ouster (String Bracket -> (mixed String Boolean)))
;; Produce the team that knocked out t in bracket br or #false if there isn't one.
  ...
  (cond [(false? br) false]
        [else
         (if (string=? t (bracket-team-lost br))
             (bracket-team-won br)
             (local [(define winner (ouster t (bracket-br-won br)))]
               (if (not (false? winner))
                   winner
