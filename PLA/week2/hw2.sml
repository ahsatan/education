...
fun get_substitutions2 (xss, s) =
    let
        fun get_subs (xss, acc) =
            case xss of
                [] => acc
              | xs :: xss' => case all_except_option(s, xs) of
                                NONE => get_subs(xss', acc)
                              | SOME(res) => get_subs(xss', acc @ res)
    in
        get_subs(xss, [])
    end
...
fun similar_names (xss, {first = f, middle = m, last = l}) =
    let
        fun gen_names (fs) =
            case fs of
                [] => []
              | f :: fs' => {first = f, middle = m, last = l} :: gen_names(fs')
    in
        gen_names(f :: get_substitutions2(xss, f))
    end
